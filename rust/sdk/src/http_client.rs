...
impl<S: Signer> crate::m10_core_client::M10CoreClient for HttpClient<S> {
    ...
    ) -> M10Result<Pin<Box<dyn Stream<Item = M10Result<Vec<AccountUpdate>>> + Send + Sync + 'static>>>
    {
        ...
        self.ws
            .observe_with_request("/accounts", req.into(), |bin| {
                let txs = FinalizedTransactions::decode(bin.as_slice()).map_err(M10Error::from)?;
    ...
    > {
        ...
        self.ws
            .observe_with_request("/transfers", req.into(), |bin| {
                FinalizedTransactions::decode(bin.as_slice()).map_err(M10Error::from)
            })
            .await
    }
    ...
    > {
        ...
        self.ws
            .observe_with_request("/metrics", req.into(), |bin| {
                TransactionMetrics::decode(bin.as_slice()).map_err(M10Error::from)
            })
            .await
    }
}
